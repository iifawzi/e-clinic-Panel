<template>
      ...
      <table class="allDoctors-content-table">
        ...
        <tr v-for="doctor in doctors" :key="doctor.doctor_id" class="allDoctors-content-table-tr">
         ...
         <td>{{doctor.phone_number}}</td>
           <td :class="doctor.avaliable === 1 ? 'green-status' : 'red-status'">{{doctor.avaliable === 1 ? $t('dashboard.forms.allDoctors.active') : $t('dashboard.forms.allDoctors.notActive')}}</td>
          <td>
 ...
 <i @click="deleteDoctor(doctor.phone_number)" class="fas fa-times options-delete"></i>
              <nuxt-link :to="'/controlPanel/dashboard/doctors/'+doctor.doctor_id"><i class="fas fa-edit allDoctors-content-table-tr-options-edit" :class="language+'-edit'"></i></nuxt-link>
              </div>
          </td>
        </tr>
      </table>
